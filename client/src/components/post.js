...
export default class Post extends Component {
  ...
  handleSubmit = event => {
    ...
    if (this.state.description && this.state.title) {
      ...
      API.createPost(post).then(dbPost => {
        this.loadPosts();
      });
    }
  };
  ...
  Validation = (valid, text) => {
    if (!valid) {
      return <label className="validation">{text}</label>;
    }
  };
        ...
        <Modal header="Create a Post" trigger={trigger}>
          <form className="post-form">
            ...
            <Textarea
              ...
              type="text"
              label="Description"
              placeholder="Description of event and contact information"
              name="description"
              onChange={this.handleInputChange}
          ...
          {this.state.submitSuccess && (
            <p className="submit-success">Post Successfully Created</p>
          )}
        </Modal>
      </Container>
    );
